...
class CodeSplitter:
    def __init__(self, ignore_extensions=None, max_chars=8000):
    ...
    def process_selected_files(self, root_dir, selected_paths, max_chars_override=None):
        """Processes only the explicitly selected files and chunks them."""
        chunks = []
        limit = max_chars_override if max_chars_override is not None else self.max_chars
        ...
        for rel_path in selected_paths:
            ...
            for idx, line in enumerate(lines):
                ...
                if len(potential_msg) > limit:
                    ...
                    if len(single_line_msg) > limit:
                        current_chunk_lines = [line[:limit - 200]]
                else:
